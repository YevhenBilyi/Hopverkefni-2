import { fetchData, updateData } from './data.js';
...
export function getNewArrays(){
   ...
   // deleting old tages before creating new ones
    ...
    for(const tag in tags){
      if(tag) {
        const array = projects.filter(item=>item.tags.includes(tags[tag]))
...
function printData(data){
    ...
    const sort = document.getElementById('sortingBy');
    sort.addEventListener('change', ()=> printData(data));

    // sorting by id
    if (sort.value === '1') data.sort((a, b) => a.id - b.id);
    
    // sorting by due
    if(sort.value === '2') data.sort((a,b) => 
        // Turn your strings into dates, and then subtract them
        // to get a value that is either negative, positive, or zero.
        new Date(b.due) - new Date(a.due)
      );
    if(sort.value === '3') data.sort((a,b) => a.priority-b.priority).reverse()
...
function createTask(obj){
        ...
        editingDiv.id = 'edit N'.concat(obj.id.toString());
...
function editTask(id){
    const parent = document.getElementById('edit N'.concat(id.toString()));
    parent.innerHTML = '';
    ...
    deleteProject.addEventListener('click', ()=>{
        ...
        updateData(myData.find(p=> p.id === id));
    ...
    updateData(myData.items[parseInt(id,10)]);
}
...
function changeProject(description, date, ar, category,title, id, priority){
   ...
   const isTrue = (priority==='true');
   myData.find(p=> p.id === id).priority = isTrue;
   if(title !== '') myData.find(p=> p.id === id).title=title;
   getNewArrays();
   document.getElementById('edit N'.concat(id.toString())).innerHTML = '';
   document.getElementById('task').innerHTML = '';
}
